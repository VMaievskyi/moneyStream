package com.piramida.entity.dao;
...
public class BasicAccountOperationTest implements ApplicationContextAware {
    ...
    private static final String ACTIVATION_STRING2 = "activationString2";
    ...
    @Test
    public void shouldFindAccountByActivationString() {
	accountDao.deleteAll();
	testAccount = initTestAccount();
	accountDao.save(testAccount);
	testAccount = initTestAccount();
	testAccount.setActivationString(ACTIVATION_STRING2);

	final Account actual = accountDao
		.findByActivationString(ACTIVATION_STRING);
	assertEquals("wrong activationString", ACTIVATION_STRING,
		actual.getActivationString());
	...
	testAccount.setStatus(ActivationStatus.ACTIVE);
	testAccount.setActivationString(ACTIVATION_STRING);
	return testAccount;
    }
